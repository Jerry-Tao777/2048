...
# collections 提供了一个字典的子类defaultdict。可以指定key值不存在时，value的默认值。
...
class GameField(object):
    ...
    def move_is_possible(self, direction):
        ...
        def row_is_left_movable(row):
            '''判断一行里面能否有元素进行左移动或者合并'''

            def change(i):
                # 当左边有空位（0），右边有数字，可以向左移动
...
def main(stdscr):
    ...
    def not_game(state):
        ...
        # 如果不是‘Restart’和‘Exit’的action，将一直保持现有状态
    ...
    state_actions = {'Init': init, 'Game': game,
                     'Win': lambda: not_game('Win'), 'Gameover': lambda: not_game('Gameover')}
    # 使用颜色配置默认值
